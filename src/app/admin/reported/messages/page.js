...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            async function getReports() {
                const res = await fetch(`/api/admin/getReportedMessages`, {
                    method: 'GET',
                    headers: {
    ...
    const handleRemove = async (report_id, message_id, target_id, reason) => {
        setRemoveModalOpen(false);
        const res = await fetch(`/api/admin/reportedMessage`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ adminId: session.user.user_id, report_id: report_id, message_id: message_id, target_id: target_id, reason: reason })
        });
    ...
    const handleOnOpenRemove = (report_id, message_id, target_id) => {
    ...
    const handleOnOpenDismiss = (report_id, message_id, target_id) => {
        setReportId(report_id);
        setMessageId(message_id);
        setUserId(target_id);
    ...
    const handleDismiss = async (report_id, message_id, target_id, reason) => {
                                ...
                                <tr key={`${report.report_id}-${index}`}>
                                    ...
                                    <td>
                                        <div className="cs4116-admin-reported-message-container">
                                            <button className="cs4116-admin-reported-message-remove-button" onClick={() => handleOnOpenRemove(report.report_id, report.message_id, report.user_id)}>Remove</button>
                                            <button className="cs4116-admin-reported-message-dismiss-button" onClick={() => handleOnOpenDismiss(report.report_id, report.message_id, report.user_id)}>Dismiss</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                    ...
                    <div className="modal-overlay">
                        <div className="modal-content">
                            ...
                            <div className="modal-buttons">
                                ...
                                <button onClick={() => {
                                    if (reportReason !== "") {
                                        handleRemove(report_id, message_id, user_id, reportReason);
                                    } else {
                    ...
                    <div className="modal-overlay">
                        <div className="modal-content">
                            ...
                            <div className="modal-buttons">
                                ...
                                <button onClick={() => {
                                    if (reportReason !== "") {
                                        handleDismiss(report_id, message_id, user_id, reportReason);
                                    } else {
                                        alert("Please select a reason before removing the message.");
                                    }
                                }}>
